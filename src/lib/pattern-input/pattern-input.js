(function ($) {
    ...
    var methods = {
                ...
                if (this.tagName == "DIV") {
                    ...
                    if (!state) {
                        ...
                        for (var j = 0; j < state.verticalDots; j++) {
                            for (var i = 0; i < state.horizontalDots; i++) {
                                var newDiv = $('<div class="dot" style="width:' + state.dotSize + 'px;' +
                                    ...
                                    (number < 10 ? ('<b><small> &nbsp&nbsp' + number + '</small></b>'): ('<b><small> &nbsp' + number + '</small></b>')) +
                                    '</div>' +
                                    '</div>');
                                $this.append(newDiv);
                        ...
                        $this.on("mousedown", function (event) {
                            ...
                            var relativeY = (event.pageY - 30) - this.offsetTop;

                            if (this.offsetParent.offsetParent.offsetParent != null) {
                                relativeX = event.pageX - (this.offsetLeft +
                                    this.offsetParent.offsetParent.offsetParent.offsetLeft +
                                    this.offsetParent.offsetParent.offsetLeft);

                                relativeY = (event.pageY - 30) - (this.offsetTop +
        ...
        setSequence: function (arrayPattern) {
            var state = this.data("patternInput");

            clearSelection(state);

            state.value = arrayPattern;
            for (i = 0; i < arrayPattern.length; i++) {
                state.dots[arrayPattern[i]].addClass("selected");
            }
            renderPath(state);
            //processTouchEnd(state);
            return this;
